...
def _rc_text(r):
    ...
    out += [f"中层管理·{m}" for m in r.get("mid") or []]
    return "、".join(out)
...
def _mid_text(s, ov, year=None):
    ...
    return "；".join(f"{b['year']}：{'、'.join(m)}" for b, m in zip(bl, mbb) if m)
