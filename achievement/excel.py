...
def write_xlsx(students, out: Path, overrides: dict):
    ...
    for s in students:
        ...
        rows.append([s["code"], s["club"], s["sid"], s["cn"], s["en"], s["cls"], len(set(s["years"])),
                     _roles_text(s, overrides), "、".join(s.get("special") or []), st["roles"], st["comm"], st["extComp"], st["intComp"], st["awards"],
                     st["extAct"], st["intAct"], st["team"], st["extSvc"], st["intSvc"], st["hours"], st["unsure"],
                     "；".join(s["issues"]), s["file"]])
    sheet(ws, ["学会代号", "学会", "学号", "姓名", "英文名", "班级", "年数", "职务（规范写法，历年）", "特别标记", "职务数", "筹委",
               "校外比赛", "校内比赛", "获奖", "校外活动", "校内活动", "团内工作", "校外服务项", "校内服务项",
               "服务时数", "待确认", "资料问题", "文件"],
          rows, [8, 14, 8, 10, 22, 7, 6, 60, 14, 7, 6, 8, 8, 6, 8, 8, 8, 9, 9, 9, 7, 30, 40])
    for r in ws.iter_rows(min_row=2, min_col=8, max_col=8):
            ...
            for k, arr in b["cats"].items():
                for i, t in enumerate(arr):
                    ...
                    rows.append([s["code"], s["sid"], s["cn"], b["year"], b.get("cls") or "",
                                 ...
                                 (b["rc"][i].get("std") or f"执委({b['rc'][i]['other']})") if k == "role" and b["rc"][i] else "",
                                 (b.get("sp", {}).get(k) or [None] * (i + 1))[i] or ""])
    sheet(ws2, ["学会", "学号", "姓名", "年份", "班级", "该年学会", "类别", "内容", "计入", "不计原因", "待确认", "获奖", "小时", "职务规范写法", "特别标记"],
          rows, [7, 8, 10, 7, 7, 18, 12, 60, 6, 30, 8, 6, 7, 20, 12])
...
执委/职务：按手册统一写法。标准职称：主席、秘书、事务（「事物」同）、财政（正/副）、查账、总学长（正/副）、助理总学长、执委、会员、队长、副队长；其它职位写成「执委(XXX,XXX)」（计入其中的职位个数）。主席、秘书、事务、财政、总学长没写正/副的当作「正」。合唱团「音乐主席」、二十四节令鼓队的队长类与执委主席同级，写成「主席(正/副)」。童军等「分团」职位、监督/督导/顾问/教练/领队类、小队长/中队长/分队长一律算「会员」。助理类、联课处工委、小教练、助教等归入执委(…)。写在执委栏的活动筹委职位（如「《弈德杯》筹委主席」）移到筹委。运动会、田径赛的职位/筹委（如「运动会主席」「监督运动会服务主席」）算服务筹委团，计入筹委（班级名义的除外）；学会在运动会上的义卖不计，演出和服务照算；高三毕联会的职位和筹委属于年级，不计（相关服务、活动照算）。会员不计职务数，同年已有其它职位时不另列会员。
